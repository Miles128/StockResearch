...
describe("MarkdownContent", () => {
  ...
  it("renders inline code", () => {
    const { container } = render(<MarkdownContent text="use `npm install` to install" />);
  ...
  it("strips <script> tags (XSS protection)", () => {
    const { container } = render(<MarkdownContent text={"<script>alert('xss')</script>"} />);
  ...
  it("strips javascript: protocol links (XSS protection)", () => {
    const { container } = render(
      <MarkdownContent text={'<a href="javascript:alert(1)">click</a>'} />,
    );
  ...
  it("renders server glossary <term> markup as styled span", () => {
    const { container } = render(
      <MarkdownContent text={'<term data-id="pe">市盈率</term> 是关键'} />,
    );
  ...
  it("strips <iframe> (XSS protection)", () => {
    const { container } = render(
      <MarkdownContent text={'<iframe src="https://evil.com"></iframe>'} />,
    );
