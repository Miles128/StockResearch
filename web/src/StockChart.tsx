...
export function StockChart({ symbol, days = 60, compact = false }: StockChartProps) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      dispose();
      try {
        const data = await api.klineChart(symbol, days);
        if (cancelled) return;
        renderCharts(data);
      } catch (e) {
    ...
    function renderCharts(data: KlineChart) {
      const textColor = getComputedStyle(document.documentElement).getPropertyValue("--bbg-text").trim() || "#e8e8e8";
      const gridColor = getComputedStyle(document.documentElement).getPropertyValue("--bbg-border").trim() || "#333";
      const common = {
        layout: {
      ...
      if (priceRef.current) {
        ...
        const chart = createChart(priceRef.current, { ...common, height: h, width: priceRef.current.clientWidth });
      ...
      if (macdRef.current) {
        const chart = createChart(macdRef.current, { ...common, height: compact ? 72 : 96, width: macdRef.current.clientWidth });
        chartsRef.current.push(chart);
      ...
      if (rsiRef.current) {
        const chart = createChart(rsiRef.current, { ...common, height: compact ? 72 : 96, width: rsiRef.current.clientWidth });
        chartsRef.current.push(chart);
