...
### Phase 12 · 预测闭环（北极星一：预测准确率）🚧 规划中

| 子项 | 说明 | 复用资产 | 验收 |
|------|------|----------|------|
| **12a 预测日记** | 研报/简报结论自动留存为预测记录（方向/置信度/horizon/标的/因子快照），到期自动评分（对/错/中性，PIT 纪律），列表+详情 | timeline、PIT、hypothesis_verify | 每份研报自动产生预测记录；到期后自动评分并可见 |
| **12b 准确率仪表盘** | 按方向/标的/维度/置信度统计历史命中率；**含错误预测的诚实展示**；校准曲线（high/medium/low 置信度的实际命中率） | 12a 数据 | 任一维度可看命中率与校准；错误预测可见 |
| **12c 白话复盘** | 「3 个月前你说过 X，实际 Y，这说明 Z」——结合场景化卡片（13a）教学 | 12a + 词库 | 到期复盘可一键生成白话解释 |
| **12d 归因学习** | 从验证数据反推各维度信号（基本面/技术/情绪/筹码）与结果的统计相关性，调整四维评分权重（替换硬编码权重） | 12a 数据 + scoring | 权重由数据驱动；变更记录可见；不改变合规语义 |
