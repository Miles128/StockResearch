...
async def prepare_capital(ctx: SectorResearchContext) -> tuple[str, str, dict[str, object]]:
    ...
    leader_lines = [f"{ld.name}({ld.symbol}) {ld.change_pct:+.2f}%" for ld in ctx.leaders[:3]]
    ...
    return (
        system,
        user,
        {"board_change": change, "leader_changes": [ld.change_pct for ld in ctx.leaders]},
    )
...
async def prepare_valuation(ctx: SectorResearchContext) -> tuple[str, str, dict[str, object]]:
    ...
    for v in valuations:
        ...
        pct_txt = (
            f"历史分位 {float(pe_pct):.0%}" if isinstance(pe_pct, int | float) else "分位不可算"
        )
...
def build_valuation(data: dict[str, object], analysis: str) -> DimensionResult:
    ...
    return finalize_dimension(
        ...
        raw_analysis=analysis,
        data_sources=["sector_leaders", "akshare_valuation"]
        if pe_available
        else ["sector_leaders"],
