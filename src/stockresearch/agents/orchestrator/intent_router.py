...
_INTENT_ROUTER_SYSTEM = (
    ...
    "- research: 用户想分析某只具体股票的基本面、技术面、估值，或该股自身风险点\n"
    "- news: 用户要看新闻、快讯，或解释/解读某条新闻、消息含义及对持仓影响\n"
    "- risk: 用户明确关心持仓/组合风控、止损、仓位体检、回撤等（不是个股分析里顺带提风险）\n"
    "- composite: 用户同时明确要求个股投研 + 持仓风控体检\n"
    "- chat: 非金融问题、闲聊、知识问答\n\n"
    "关键区分规则：\n"
    "- 「大盘/市场/股市/指数/行情/走势/板块/A股」→ market（不要标成 risk）\n"
    "- 「某只股票+分析/研究/怎么样/风险点」→ research（个股风险≠持仓风控）\n"
    "- 「新闻/快讯/消息/发生了什么/解释这条新闻/对持仓有什么影响」→ news\n"
    "- 仅当明确「我的持仓/组合 + 风险/止损/体检/回撤/安全」→ risk\n"
    "- 非金融话题 → chat\n\n"
    "请以 JSON 格式回复：\n"
...
_RESEARCH_PATTERNS = ("分析", "研究", "值不值得", "怎么样", "看看", "评", "基本面", "技术面", "风险点", "有什么风险")
_PORTFOLIO_RISK_PATTERNS = (
    "持仓风险",
    "持仓安全",
    "持仓体检",
    "仓位风险",
...
def _is_portfolio_risk(text: str) -> bool:
    if any(p in text for p in _PORTFOLIO_RISK_PATTERNS):
        if any(
            p in text
            for p in ("风险", "止损", "回撤", "体检", "安全", "危险", "风控", "仓位", "持仓", "组合")
        ):
            return True
    if "体检" in text and ("风控" in text or "持仓" in text or "仓位" in text):
        return True
    return False
...
def _keyword_fallback(message: str) -> tuple[str, list[str], list[str]]:
    ...
    portfolio_risk = _is_portfolio_risk(text)

    if portfolio_risk and symbols and any(p in text for p in _RESEARCH_PATTERNS):
        return INTENT_COMPOSITE, symbols, []
    if portfolio_risk and not symbols:
    ...
    # Stock-level "风险" without portfolio language → research, not risk checkup.
    if symbols and ("风险" in text or "危险" in text):
        return INTENT_RESEARCH, symbols, []
