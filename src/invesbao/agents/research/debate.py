...
from invesbao.agents.voice import DEBATE_ROUNDS, DEBATE_VOICE, JUDGE_VOICE
from invesbao.core.constants import DISCLAIMER
from invesbao.core.schemas import DebateResult, DebateRound, DimensionResult
    ...
    *,
    ...
    for voter_id, voter_name, side_label in (
        ...
        raw = await llm.complete(
            f"你是{side_label}方。基于整场辩论，只输出一个词：偏多、偏空或中性。",
            debate_transcript,
        )
    ...
    for round_num in range(1, rounds + 1):
        ...
        if round_num == 1:
            ...
            async def fetch_side(
                ...
                prompt = f"{context}\n第{round_num}轮：{task}"
                text = (await llm.complete(system, prompt)).strip()
            ...
            side_tasks = [
                asyncio.create_task(
                    fetch_side(agent_id, agent_name, role, system, side_label)
                )
                for agent_id, agent_name, role, system, side_label in agents
            ]
            async for event in iter_merged_agent_streams_from_tasks(side_tasks):
            ...
            for agent_id, agent_name, role, system, side_label in agents:
                ...
                text = (await llm.complete(system, prompt)).strip()
                round_texts[agent_id] = text
                transcript.append(f"第{round_num}轮{side_label}：{text}")
                side_key = "neutral" if agent_id == "neutral" else agent_id
                pumps.append(
                    asyncio.create_task(
                        pump_agent_done_stream(
